...
public class AirportRouter extends Router {
	...
	void determineRoute(Parcel parcel) throws JitsException {
		...
		Coordinate fromZipCoord = Router.zipcodeMap.get(parcel.getFromZip());
		Coordinate toZipCoord = Router.zipcodeMap.get(parcel.getToZip());

		if ((fromZipCoord.distanceTo(toZipCoord) > 800.0)) {
			parcel.getRoute().add(2, airportList.get(3));
			
			if ((Long.parseLong(parcel.getFromZip()) <= 80000) & (Long.parseLong(parcel.getToZip()) <= 8000)
					|| (Long.parseLong(parcel.getFromZip()) > 80000) & (Long.parseLong(parcel.getToZip()) > 8000)) {
				parcel.getRoute().remove(2);
			}
		}
